<!--
  A real LOOP_STATE.md, copied verbatim from max-friedman/tactbench after four
  rounds. Nothing has been cleaned up for presentation — including round 1, which
  records that the project's central claim was measured and found false.

  Read it next to templates/LOOP_STATE.template.md to see what each section looks
  like once it is carrying real work. The narrative version, with what each round
  bought, is in docs/CASE_STUDY.md.
-->
